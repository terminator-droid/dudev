...
(id SERIAL PRIMARY KEY,
 category_id INTEGER REFERENCES category (id),
    ...
    created_at TIMESTAMP, user_id INTEGER NOT NULL REFERENCES users (id) ON DELETE CASCADE,
...
(id SERIAL PRIMARY KEY REFERENCES product (id) ON DELETE CASCADE,
    model VARCHAR (32) NOT NULL,
    shop_price DOUBLE PRECISION, created_at TIMESTAMP NOT NULL);
...
(id SERIAL PRIMARY KEY,
 offer_id INTEGER NOT
 NULL REFERENCES offer (id) ON DELETE CASCADE,
...
(id SERIAL PRIMARY KEY,
 user_id INTEGER NOT
 NULL REFERENCES users (id) ON DELETE CASCADE,
    product_id INTEGER NOT NULL REFERENCES product (id) ON DELETE CASCADE,
    created_at TIMESTAMP NOT NULL);